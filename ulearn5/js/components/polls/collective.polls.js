! function($) {
  ...
  $(function() {
    var init_poll = function($poll, $viewResult) {
      var url = $poll.attr('data-poll-url');

      if ($viewResult) {
        $poll.attr('data-poll-viewResult', 0);
      } else {
        $poll.attr('data-poll-viewResult', 1);
      }
      ...
      $.ajax({
        ...
        success: function(data) {
          var $poll = this;
          var viewResult = $poll.attr('data-poll-viewResult') == 0;
          ...
          if (has_votted || closed || viewResult) {
  ...
  DrawPoll.prototype = {
    ...
    setup_default: function(type) {
      var width = $(".ulearnbox.portlet-polls dl").width();
      switch (type) {
        case 'pie':
